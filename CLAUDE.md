...
Built with **Astro (SSR, server output)** + **Preact islands** + **Tailwind CSS v4** + the suite design system (`data-app="climb"`, green accent — see [`../docs/DESIGN.md`](../docs/DESIGN.md)). Data lives in Postgres (see `db/`).
...
- `BaseLayout.astro` passes a "Log in"/"Log out" URL (auth service, `?redirect=` back to the current page) to the suite bar's `authHref`; hidden on localhost.
...
src/
  ...
  components/
    lampham/                    # GENERATED by ../scripts/sync-design.sh — SuiteHead/Header/Footer,
                                  # don't edit here (see ../docs/DESIGN.md)
    table/
  ...
  styles/
    global.css               # Tailwind entry: imports ./lampham (design system) + .th/.td/.field + grade pills
```
...
- Look comes from the suite design system — `src/styles/lampham/` and `src/components/lampham/` are **generated** copies of the root `design/` dir (`bash ../scripts/sync-design.sh climbing-tracker`); change the design there, never here. Tokens, `lp-*` classes, suite bar/footer: [`../docs/DESIGN.md`](../docs/DESIGN.md).
- `<html data-app="climb">` selects the green accent. Tables are wrapped in `lp-compact` (dense data view); stat cards are `lp-card` via `StatCard.astro`.
- `global.css` only holds app-local styles on top of that:
...
Suite-wide colours/fonts live in the root `design/` (see `../docs/DESIGN.md`); the only app-local colours are the grade pills in `global.css`.
